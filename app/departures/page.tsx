...
export default function DeparturesPage() {
  ...
  useEffect(() => {
    const loadFlights = async () => {
      try {
        ...
        const filteredFlights = filterDepartedFlights(data.departures);
  ...
  const filterDepartedFlights = (allFlights: Flight[]): Flight[] => {
    const now = new Date();
    ...
    return [...activeFlights, ...recentDepartedFlights].sort((a, b) => {
  ...
  const getStatusColor = (status: string): string => {
    const statusLower = status.toLowerCase();
    if (statusLower.includes('departed') || statusLower.includes('poletio')) return 'text-green-400';
...
const formatTerminal = (terminal?: string): string => {
  if (!terminal) return '-';
  // Convert T01 to T1, T02 to T2, etc.
  return terminal.replace('T0', 'T').replace('T', 'T ');
};

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white p-4 transition-all duration-500">
      {/* Header */}
      <div className="w-[95%] mx-auto mb-6">
        <div className="flex flex-col lg:flex-row justify-between items-center gap-4 mb-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-4xl lg:text-5xl font-black bg-gradient-to-r from-yellow-400 to-orange-400 bg-clip-text text-transparent">
                DEPARTURES
              </h1>
        ...
        <div className="flex flex-wrap gap-2 mb-4 justify-center">
          {[
            ...
            { label: 'Boarding', color: 'blue' },
            { label: 'Departed', color: 'green' },
        ...
        {loading && flights.length === 0 ? (
          <div className="text-center p-12">
            <div className="inline-flex items-center gap-4">
              ...
              <span className="text-lg text-slate-300">Loading departure information...</span>
            </div>
          </div>
        ) : (
            ...
            <div className="divide-y divide-white/5 max-h-[75vh] overflow-y-auto">
              {flights.length === 0 ? (
                <div className="p-8 text-center text-slate-400">
                  ...
                  <div className="text-base">No departures scheduled</div>
                </div>
              ) : (
...
<div className="col-span-2">
  ...
  {flight.EstimatedDepartureTime && 
   flight.EstimatedDepartureTime !== flight.ScheduledDepartureTime && (
    <div className="text-xs text-yellow-400 animate-blink mt-0.5">
      Est: {flight.EstimatedDepartureTime}
                      ...
                      {/* Destination - BIGGER FONT as requested */}
