...
import { CheckCircle, Clock, MapPin, Users, AlertCircle, Info } from 'lucide-react';
...
const AD_CHANGE_INTERVAL = 15000; // 15 seconds

// ============================================================================
// UTILITY COMPONENTS - Lagane, ne zahtevaju memoizaciju
// ============================================================================
...
const DeskNumberDisplay = ({ deskNumberParam, isPortrait }: { deskNumberParam: string; isPortrait: boolean }) => (
...
const NextFlightInfo = ({ nextFlight, isPortrait }: { nextFlight: Flight | null; isPortrait: boolean }) => {
...
// ============================================================================
// HEAVY COMPONENTS - Memoizovane zbog Image loading-a i kompleksnosti
// ============================================================================
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-4 text-white">
        <div className={`text-center bg-white/10 backdrop-blur-md rounded-3xl p-8 sm:p-12 border border-white/20 shadow-2xl ${
          isPortrait ? 'max-w-4xl' : 'max-w-6xl'
        } mx-auto`}>
          <CheckCircle className="w-24 h-24 sm:w-32 sm:h-32 text-white/60 mx-auto mb-6 sm:mb-8" />
          
          <DeskNumberDisplay deskNumberParam={deskNumberParam} isPortrait={isPortrait} />
          
          <div className={`text-white/90 mb-6 font-semibold ${
            isPortrait ? 'text-3xl sm:text-4xl' : 'text-2xl sm:text-3xl'
          }`}>
            {displayFlight ? 'Check-in not available' : 'No flights currently checking in here'}
          </div>

          <NextFlightInfo nextFlight={nextFlight} isPortrait={isPortrait} />

          <div className={`text-white/80 mb-6 ${
            isPortrait ? 'text-xl sm:text-2xl' : 'text-lg sm:text-xl'
          }`}>
            {displayFlight ? `Status: ${displayFlight.StatusEN}` : 'Please check the main display'}
          </div>

          <div className={`text-white/70 mb-4 ${
            isPortrait ? 'text-lg sm:text-xl' : 'text-base sm:text-lg'
          }`}>
          ...
          {loading && (
            <div className={`text-white/60 mt-4 ${
              isPortrait ? 'text-base sm:text-lg' : 'text-sm sm:text-base'
            }`}>
              Updating...
            </div>
          )}
...
InactiveState.displayName = 'InactiveState';

interface PortraitViewProps {
  safeDisplayFlight: Flight;
  deskNumberParam: string;
  lastUpdate: string;
  loading: boolean;
  adImages: string[];
    ...
    <div className="p-3 bg-white/5 backdrop-blur-lg border-b border-white/10 mt-[1cm]">
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          ...
          {loading && <div className="text-xs text-slate-500 mt-1">Updating...</div>}
        </div>
      </div>
    </div>
...
PortraitView.displayName = 'PortraitView';
...
LandscapeView.displayName = 'LandscapeView';
...
export default function CheckInPage() {
  ...
  // Memoized desk number variants - OPTIMIZACIJA
  const deskNumberVariants = useMemo(() => {
  ...
  // Refs za cleanup - ISPRAVLJENO: Inicijalne vrednosti
  ...
  // Optimizovana orientation detection sa debounce
  useEffect(() => {
  ...
  // Optimizovana funkcija za učitavanje letova
  const loadFlights = useCallback(async () => {
    try {
      ...
      // Pronađi letove za sve varijante broja šaltera
      ...
      if (validProcessingFlights.length > 0) {
        ...
        // Deep comparison za garanciju - sprečava state thrashing
  ...
  // Flight data polling sa cleanup
  useEffect(() => {
    loadFlights();
    
    flightIntervalRef.current = setInterval(loadFlights, REFRESH_INTERVAL);
  ...
  // Ad rotation - samo ako ima više od 1 slike
  useEffect(() => {
  ...
  // Memoized values
  const displayFlight = useMemo(() => currentData || flight, [currentData, flight]);
  const shouldShowCheckIn = useMemo(
    () => displayFlight && displayFlight.StatusEN?.toLowerCase() === 'processing',
    [displayFlight]
  );

  // Early returns za optimalne performanse
  ...
  if (isPortrait) {
    return (
      <PortraitView
        safeDisplayFlight={safeDisplayFlight}
        deskNumberParam={deskNumberParam}
        lastUpdate={lastUpdate}
        loading={loading}
        adImages={adImages}
